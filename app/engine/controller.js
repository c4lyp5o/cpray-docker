...
//     cpray.getTimesbyMonth(req.params.zone)
//         .then(data => {
...
exports.knowingHome = async (req, res) => {
    const zoneReplace = {
        ...
        if (value === null || value.length === 0) {
            const timeValue = await cpray.getTimesbyMonth(req.params.zone);
            ...
            res.render('index', { data: redisVal, zone: realZone });
            console.log('no redis cache. now caching');
        }
        else {
            console.log('using redis cache');            
            const realZone = zoneReplace[req.params.zone];
            res.render('index', { data: value, zone: realZone });
        }
    }
